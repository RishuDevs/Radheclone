...
from RadheMusic import YouTube, app
from RadheMusic.core.call import RadheMusic
from RadheMusic.misc import SUDOERS, db
...
from RadheMusic.utils.formatters import seconds_to_min
...
from RadheMusic.utils.stream.autoclear import auto_clean
from RadheMusic.utils.thumbnails import get_thumb
from config import (
    BANNED_USERS,
    SOUNCLOUD_IMG_URL,
    STREAM_IMG_URL,
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await RadheMusic.pause_stream(chat_id)
        buttons = [
            [
                InlineKeyboardButton(
        ...
        await RadheMusic.resume_stream(chat_id)
        buttons_resume = [
            [
                InlineKeyboardButton(
        ...
        await CallbackQuery.answer()
        await RadheMusic.stop_stream(chat_id)
        ...
        await RadheMusic.mute_stream(chat_id)
        ...
        await RadheMusic.unmute_stream(chat_id)
        ...
        if command == "Skip":
            ...
            try:
                ...
                if not check:
                    ...
                    try:
                        return await RadheMusic.stop_stream(chat_id)
                    except:
                        return
            except:
                try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await RadheMusic.skip_stream(chat_id, link, video=status, image=image)
            except:
            ...
            try:
                ...
                image = None
            try:
                await RadheMusic.skip_stream(chat_id, file_path, video=status, image=image)
            except:
                return await mystic.edit_text(_["call_6"])
            ...
            try:
                await RadheMusic.skip_stream(chat_id, videoid, video=status)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            button = stream_markup2(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await RadheMusic.skip_stream(chat_id, queued, video=status, image=image)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            if videoid == "telegram":
                button = stream_markup2(_, chat_id)
