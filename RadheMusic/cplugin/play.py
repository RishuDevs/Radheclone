...
import config
from RadheMusic import Apple, Resso, SoundCloud, Spotify, Telegram, YouTube, app
from RadheMusic.core.call import RadheMusic
from RadheMusic.misc import SUDOERS
from RadheMusic.utils.inline import panel_markup_clone
from RadheMusic.utils import seconds_to_min, time_to_seconds
from RadheMusic.utils.channelplay import get_channeplayCB
from RadheMusic.utils.decorators.language import languageCB
from RadheMusic.utils.decorators.play import CPlayWrapper
from RadheMusic.utils.formatters import formats
from RadheMusic.utils.inline import (
    botplaylist_markup,
    livestream_markup,
    playlist_markup,
    slider_markup,
...
from RadheMusic.utils.database import (
...
from RadheMusic.utils.logger import play_logs
from config import BANNED_USERS, lyrical
from time import time
from RadheMusic.utils.extraction import extract_user
...
async def play_commnd(
    ...
    if audio_telegram:
        ...
        if await YouTube.exists(url):
            ...
            try:
                await RadheMusic.stream_call(url)
...
import config
from RadheMusic import Carbon, YouTube
from RadheMusic.core.call import RadheMusic
from RadheMusic.misc import db
from RadheMusic.utils.database import add_active_video_chat, is_active_chat
from RadheMusic.utils.exceptions import AssistantErr
from RadheMusic.utils.inline import (
    aq_markup,
    queuemarkup,
    close_markup,
...
from RadheMusic.utils.pastebin import RadheMusicBin
from RadheMusic.utils.stream.queue import put_queue, put_queue_index
from youtubesearchpython.__future__ import VideosSearch
from RadheMusic.utils.database.clonedb import get_owner_id_from_db, get_cloned_support_chat, get_cloned_support_channel
...
async def stream(
    ...
    if not result:
        return
    if forceplay:
        await RadheMusic.force_stop_stream(chat_id)
    if streamtype == "playlist":
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await RadheMusic.join_call(chat_id, original_chat_id, file_path, video=None)
            await put_queue(
                chat_id,
                original_chat_id,
                file_path,
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await RadheMusic.join_call(chat_id, original_chat_id, file_path, video=status)
            await put_queue(
                chat_id,
                original_chat_id,
                file_path,
